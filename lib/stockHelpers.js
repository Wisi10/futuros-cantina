...
export function effectiveThreshold(product, globalThreshold = 10) {
  const perProduct = Number(product?.low_stock_alert);
  if (Number.isFinite(perProduct) && perProduct > 0) return perProduct;
  return Number(globalThreshold) > 0 ? Number(globalThreshold) : 10;
}
...
export function isLowStock(product, globalThreshold = 10) {
