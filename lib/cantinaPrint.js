...
function getRate(sale, rates) {
  const refRate = Number(sale?.exchange_rate_bs || rates?.eur || 0);
  const usdRate = Number(sale?.exchange_rate_usd || rates?.usd || refRate);
  return { eur: refRate, usd: usdRate };
}

function fmtMoney(ref, currency, rate) {
  if (currency === "usd") {
    const usdAmount = rate.usd > 0 ? (Number(ref) * rate.eur) / rate.usd : 0;
    return fmtUsd(usdAmount);
  }
  return fmtBs(Number(ref) * rate.eur);
}
...
export function generateCantinaReceipt(sale, { rates, currency = "bs", businessInfo = null, payments = null } = {}) {
  ...
  const html =
    ...
    "<div class=\"total-secondary\">" + fmtRef(totalRef) + "</div>" +
    "</div>" +
...
export function generateCantinaInvoice(sale, { invoiceNumber, rates, currency = "bs", businessInfo = null, payments = null } = {}) {
  ...
  const html =
    ...
    "</table>" +
    "<div class=\"totals\">" +
    "<div class=\"row\"><span>Subtotal:</span><span>" + fmtMoney(subtotalRef, currency, rate) + " <span style=\"color:#888\">(" + fmtRef(subtotalRef) + ")</span></span></div>" +
    "<div class=\"row\"><span>IVA (16%):</span><span>" + fmtMoney(ivaRef, currency, rate) + " <span style=\"color:#888\">(" + fmtRef(ivaRef) + ")</span></span></div>" +
    "<div class=\"row total-row\"><span>TOTAL:</span><span>" + fmtMoney(totalRef, currency, rate) + " <span style=\"color:#888\">(" + fmtRef(totalRef) + ")</span></span></div>" +
    "</div>" +
