...
// Nota: cantina cobra en USD. La columna `price_ref` y similares guardan
// el valor en USD (label histórico "REF" era un despiste). Este helper
// formatea como "$X.XX". Eventos y rate displays mantienen su label REF.
export const formatREF = (n) => `$${Number(n || 0).toFixed(2)}`;
...
// Cantina cobra en USD. Estos helpers convierten USD → REF/EUR (display
// secundario para quien sigue pensando en REF). REF = USD × usd_rate / eur_rate.
export const calcREFsec = (usd, rate) => {
  if (!rate?.eur || !rate?.usd) return null;
  return Number(usd || 0) * rate.usd / rate.eur;
};

export const formatREFsec = (usd, rate) => {
  const ref = calcREFsec(usd, rate);
  if (ref == null) return "—";
  return `REF ${ref.toFixed(2)}`;
};

// Legacy: cuando el primary era REF y queríamos mostrar el equivalente USD.
// Ya no aplica; mantenido solo por compatibilidad si algún call externo lo usa.
export const calcUSD = (ref, rate) => {
  if (!rate?.eur || !rate?.usd) return null;
  return Number(ref || 0) * rate.eur / rate.usd;
};

export const formatUSD = (ref, rate) => {
  const usd = calcUSD(ref, rate);
  if (usd == null) return "—";
...
// Note: DB uses "Snacks" plural — keep aligned. Used as fallback when the
// product_categories table is unreachable or empty.
export const CANTINA_CATEGORIES = ["Bebida", "Comida", "Snacks", "Otro"];

// Fetch categories from product_categories (source of truth, configurable via
// Config > Categorías). Falls back to CANTINA_CATEGORIES on error/empty.
export async function loadProductCategoryNames(supabaseClient) {
  if (!supabaseClient) return CANTINA_CATEGORIES;
  try {
    const { data, error } = await supabaseClient
      ...
      .order("sort_order")
      .order("name");
    if (error || !data || data.length === 0) return CANTINA_CATEGORIES;
...
// Las 5 categorías oficiales del tab Gastos (acordadas con Sam, junio 2026).
// "Materia Prima / Insumos" la usan los auto-inserts (restocks, pagos por
// pagar, wizard producto). "Activos" es para compras de equipos (vitrina,
// freezer, tablet POS, etc.) que NO son consumibles. Las otras 3 son para
// gastos manuales (nómina, servicios, mantenimiento/otros).
...
// needsRef: bloquea guardar si esta vacio (input obligatorio)
// acceptsRef: muestra input pero permite guardar vacio (input opcional)
// refHint: texto del placeholder cuando el input aparece
export const PAYMENT_METHODS = [
  { id: "datafono", label: "Tarjeta", icon: "💳", needsRef: false },
  { id: "pago_movil", label: "Pago Movil", icon: "📱", needsRef: false, acceptsRef: true, refHint: "Últimos 4 dígitos" },
  { id: "cash_bs", label: "Efectivo Bs", icon: "💵", needsRef: false },
  { id: "cash_usd", label: "Cash USD", icon: "💲", needsRef: false },
  { id: "zelle", label: "Zelle", icon: "🏦", needsRef: true, acceptsRef: true },
